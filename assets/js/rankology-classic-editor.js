    ...
    $(document).on('wplink-open', function (event, wrap) {
        ...
        if (!inputs.sponsored) {
            $('#link-options').append(
                ...
                <label><span></span>
                <input type="checkbox" id="wp-link-sponsored" />${rankologyI18n.sponsored}</label>
            </div>`
            );
        }
        if (!inputs.nofollow) {
            $('#link-options').append(
                `<div class="link-no-follow">
                    <label><span></span>
                    <input type="checkbox" id="wp-link-no-follow" />${rankologyI18n.nofollow}</label>
                </div>`
            );
        }
        if (!inputs.ugc) {
            $('#link-options').append(
                `<div class="link-ugc">
                    <label><span></span>
                    <input type="checkbox" id="wp-link-ugc" />${rankologyI18n.ugc}</label>
                </div>`
            );
        }
