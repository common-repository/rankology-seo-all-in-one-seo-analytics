...
class CustomCapabilities implements ExecuteHooksBackend
{
    ...
    public function hooks()
    {
        if ('1' == rankology_get_toggle_option('advanced')) {
            add_filter('rankology_capability', [$this, 'custom'], 9999, 2);
            add_filter('option_page_capability_rankology_titles_option_group', [$this, 'capabilitySaveTitlesMetas']);
            add_filter('option_page_capability_rankology_xml_sitemap_option_group', [$this, 'capabilitySaveXmlSitemap']);
            add_filter('option_page_capability_rankology_social_option_group', [$this, 'capabilitySaveSocial']);
            add_filter('option_page_capability_rankology_google_analytics_option_group', [$this, 'capabilitySaveAnalytics']);
            add_filter('option_page_capability_rankology_instant_indexing_option_group', [$this, 'capabilitySaveInstantIndexing']);
            add_filter('option_page_capability_rankology_advanced_option_group', [$this, 'capabilitySaveAdvanced']);
            add_filter('option_page_capability_rankology_tools_option_group', [$this, 'capabilitySaveTools']);
            add_filter('option_page_capability_rankology_import_export_option_group', [$this, 'capabilitySaveImportExport']);

            add_filter('option_page_capability_rankology_fno_mu_option_group', [$this, 'capabilitySavePro']);
            add_filter('option_page_capability_rankology_fno_option_group', [$this, 'capabilitySavePro']);
            add_filter('option_page_capability_rankology_bot_option_group', [$this, 'capabilitySaveBot']);
    ...
    public function addCapabilities()
    {
        ...
        if (isset($roles->role_objects['administrator'])) {
            ...
            foreach ($pages as $value) {
                $role->add_cap(\sprintf('rankology_manage_%s', $value), true);
            }
        }
        ...
        $needle  = 'rankology_advanced_security_metaboxe';
        ...
        foreach ($pages as $key => $pageValue) {
            ...
            if (! \array_key_exists($optionKey, $options)) {
                ...
                foreach ($roles->role_objects as $keyRole => $role) {
                    ...
                    $role->remove_cap(\sprintf('rankology_manage_%s', $capability));
                }
            } else {
                foreach ($roles->role_objects as $keyRole => $role) {
                    if (! \array_key_exists($role->name, $options[$optionKey]) && 'administrator' !== $keyRole) {
                        ...
                        $role->add_cap(\sprintf('rankology_manage_%s', $capability), true);
                    }
                }
            }
        }
