...
namespace Rankology\Actions\Ajax;
...
class PreviewMetaTitle implements ExecuteHooksBackend {
    ...
    public function get() {
        ...
        $contextPage = rankology_get_service('ContextPage')->buildContextWithCurrentId((int) $_GET['post_id']);
        if ($postId) {
        ...
        $value   = rankology_get_service('TagsToString')->replace($template, $contextPage->getContext());
