...
class PreviewTitleDescriptionMeta implements ExecuteHooks {
    ...
    public function register() {
        register_rest_route('rankology/v1', '/posts/(?P<id>\d+)/preview-title-description-metas', [
            'methods'             => 'GET',
    ...
    public function preview(\WP_REST_Request $request) {
        ...
        $post_date = '';
        if (rankology_get_service('TitleOption')->getSingleCptDate($id)) {
        ...
        $contextPage = rankology_get_service('ContextPage')->buildContextWithCurrentId($id);
        ...
        $title         = rankology_get_service('TagsToString')->replace($title, $contextPage->getContext());
        $description   = rankology_get_service('TagsToString')->replace($description, $contextPage->getContext());
