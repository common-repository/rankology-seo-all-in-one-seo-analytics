...
class CountTargetKeywordsUse
{
    public function getCountByKeywords($targetKeywords, $postId = null)
    {
        ...
        $cached = get_transient('rankology_content_analysis_count_target_keywords_use_' . $hashed);
        if(false !== $cached){
            return $cached;
        }
        ...
        FROM {$wpdb->postmeta}
        WHERE meta_key = '_rankology_analysis_target_kw'
        ...
        set_transient('rankology_content_analysis_count_target_keywords_use_' . $hashed, $data, 5 * MINUTE_IN_SECONDS);
