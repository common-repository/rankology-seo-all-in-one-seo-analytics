...
class FacebookImageOptionMeta {
    ...
    public function getUrl(){
        if (function_exists('is_shop') && is_shop()) {
            $value = get_post_meta(get_option('woocommerce_shop_page_id'), '_rankology_social_fb_img', true);
        } else {
            $value = get_post_meta(get_the_ID(), '_rankology_social_fb_img', true);
        }

        if(empty($value) &&  '1' === rankology_get_service('SocialOption')->getSocialFacebookImgDefault() ){
            $options = get_option('rankology_social_option_name');
            $value = isset($options['rankology_social_facebook_img']) ? $options['rankology_social_facebook_img'] : null;
        }
    ...
    public function getAttachmentId(){
        if (function_exists('is_shop') && is_shop()) {
            $value = get_post_meta(get_option('woocommerce_shop_page_id'), '_rankology_social_fb_img_attachment_id', true);
        } else {
            $value = get_post_meta(get_the_ID(), '_rankology_social_fb_img_attachment_id', true);
        }

        if(empty($value) &&  '1' === rankology_get_service('SocialOption')->getSocialFacebookImgDefault() && empty(get_post_meta(get_the_ID(), '_rankology_social_fb_img', true)) ){
            $options = get_option('rankology_social_option_name');
            $value = isset($options['rankology_social_facebook_img_attachment_id']) ? $options['rankology_social_facebook_img_attachment_id'] : null;
        }
