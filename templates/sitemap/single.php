...
// Sanitize and validate the query var
$path = sanitize_text_field(get_query_var('rankology_cpt'));

// Sanitize the $offset and validate it
$offset = basename(parse_url($_SERVER['REQUEST_URI'], PHP_URL_PATH), '.xml');
$offset = preg_match_all('/\d+/', $offset, $matches);
$offset = !empty($matches[0]) ? (int)end($matches[0]) : 0;
...
$offset = ($offset > 0) ? (($offset - 1) * $max) : 0;
...
// Archive link
if (get_post_type_archive_link($path) && $offset === 0) {
                ...
                if (!empty($language_list)) {
                    foreach ($language_list as $key => $language_infos) {
                        if ($original_language !== $language_infos['language_code']) {
                            ...
                            $archive_links[] = esc_url(user_trailingslashit(get_post_type_archive_link($path)));
