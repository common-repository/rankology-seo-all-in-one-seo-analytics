...
echo rankology_common_esc_str($request->getUrl());
// >> http://www.foo.com/
```
...
$client->addSubscriber(new LogPlugin(new ClosureLogAdapter(function($m) { echo rankology_common_esc_str($m); }, $verbosity = LogPlugin::LOG_VERBOSE);
```
...
$client->addSubscriber(new LogPlugin(new ClosureLogAdapter(function($m) { echo rankology_common_esc_str($m); }, $format = MessageFormatter::DEBUG_FORMAT);
```
