...
$promise->then(null, function ($reason) {
    echo rankology_common_esc_str($reason);
});
...
echo rankology_common_esc_str($promise->wait()); // outputs "foo"
```
...
rankology_common_esc_str($promise->wait()); // throws the exception.
```
...
```php
$promise = new Promise();
$promise->then(function ($value) { echo esc_html($value); });
