...
echo rankology_common_esc_str($composed); // abc, 123. Above all listen to me.
```
...
$stream->read(1024);
echo rankology_common_esc_str($stream->tell());
// 1024
...
echo rankology_common_esc_str($stream); // 0123456789
```
...
echo rankology_common_esc_str($stream->read(10));
...
echo rankology_common_esc_str($original->getSize());
// >>> 1048576

// Limit the size of the body to 1024 bytes and start reading from byte 2048
$stream = new Psr7\LimitStream($original, 1024, 2048);
echo rankology_common_esc_str($stream->getSize());
// >>> 1024
echo rankology_common_esc_str($stream->tell());
// >>> 0
```
...
echo rankology_common_esc_str($noSeek->read(3));
// foo
